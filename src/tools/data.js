...
export function registerDataTools(server) {
  ...
  }, handle(core.getIndicator));

  server.tool('data_get_strategy_results', 'Get strategy performance metrics from Strategy Tester', {}, handle(core.getStrategyResults));
  ...
  }, handle(core.getTrades));

  server.tool('data_get_equity', 'Get equity curve data from Strategy Tester', {}, handle(core.getEquity));
  ...
  }, handle(core.getQuote));

  server.tool('depth_get', 'Get order book / DOM (Depth of Market) data from the chart', {},
    handle(core.getDepth, { hint: 'Open the DOM panel in TradingView before using this tool.' }));
  ...
  }, handle(core.getPineBoxes));

  server.tool('data_get_study_values', 'Get current indicator values from the data window for all visible studies (RSI, MACD, Bollinger Bands, EMAs, custom indicators with plot()).', {}, handle(core.getStudyValues));
}
